...
require_once './ClsCategory.php';
	if (isset($_POST['reportType']))
	{
		...
		{
			$qry = "select month(dtExpenseDate), sum(uAmount) as total from tbldailyexpense e, tblcategories c where dtExpenseDate >= '" . $startDate . "' and dtExpenseDate <= '" . $endDate . "' and e.uCategoryId = c.uCategoryId group by month(dtExpenseDate);";
		}
		$con = $config->connect();
